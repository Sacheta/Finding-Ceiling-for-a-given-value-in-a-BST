//program for simple execution of a function to find the Ceil of a value in a BST
#include<iostream>
#include<climits>
using namespace std;
...
void node::ceil(node* root,int n){  //function to find the floor of a value in a given BST
    if(root==nullptr){  //if tree's empty
        cout<<"Empty tree!"<<endl;
        return;
    }
    int res=INT_MAX;  //assigning the maximum possible value to int
    while(root!=nullptr){  //until we don't get to end of the tree
        if(root->data==n){  //if root has the required value
            ...
            res=root->data;  //if more closer value to the value required, can be found
            root=root->left;
        }
    }
...
int main(){
    ...
    cout<<"5. Display ceil of an element"<<endl;
